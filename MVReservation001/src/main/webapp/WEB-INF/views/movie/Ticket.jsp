...
<body>
	...
	<main id="main" class="main">
		...
		<div class="pagetitle">
			<h1>movie/Ticket.jsp :: ${param.selectmovie } </h1>
					...
					<div class="card">
						<div class="card-body">
							...
							<div class="list-group reserveArea" id="mvListArea"  >
								...
								<button  id="${movie.mvcode }" type="button" class="list-group-item list-group-item-action"
								        onclick="selectMovie(this, '${movie.mvcode }', '${movie.mvpos }' )"><span class="listTitle font-bold" title="${movie.mvtitle }">${movie.mvtitle }</span></button>
					...
					<div class="card">
						<div class="card-body">
							...
							<div class="list-group reserveArea" id="thListArea">
								...
								<button id="${theater.thcode }" type="button" class="list-group-item list-group-item-action" 
				...
				<div class="col-12">
					<div class="card mb-0">
						<div class="card-body p-3">
							<div class="row" >
								<div class="col" style="text-align: center; max-width: 200px;">
									<img id="CheckMvPoster" style="max-height: 120px;" >
									<h4 class="card-title pt-2 pb-1 listTitle mb-0" id="CheckMvTitle"></h4>
								</div>
																					
								<div class="col ">
									<h4>극장 <span id="CheckThName"></span></h4>
								...
								<div class="col" style="text-align: center; min-width:250px; max-width: 250px;">
									<button class="btn btn-danger font-bold mb-1" style="width: 150px; height: 150px;"
	...
	<script type="text/javascript">
		$(document).ready(function(){
			 var selMV = '${param.selectmovie }';
			 var selTH = '${param.selecttheater }';
			 var selSCDATE = '${param.selectdate }';
			 var selSCROOMTIME = '${param.selectroomtime }';
			 if(selMV.length > 0){
				 $("#"+selMV).attr('tabindex', '-1').focus();
				 $("#"+selMV).click();
				 
				 if(selTH.length > 0){
					 $("#"+selTH).attr('tabindex', '-1').focus();
					 $("#"+selTH).click();		
					 if(selTH.length > 0){
						 $("#"+selSCDATE).attr('tabindex', '-1').focus();
						 $("#"+selSCDATE).click();		
						 if(selSCROOMTIME.length > 0){
							 console.log("selSCROOMTIME:" + selSCROOMTIME)
							 $("#"+selSCROOMTIME).attr('tabindex', '-1').focus();
							 $("#"+selSCROOMTIME).click();		
						 }
	...
	function selectMovie(selBtn,mvcode, mvpos){ //영화 목록에서 영화 선택 할 경우 호출 
			...
			success:function(theaterList){
				...
				for(var thinfo of theaterList){
					if(thinfo.checkth == null){
						...
						output += "<button type='button' id=\""+thinfo.thcode+"\" class='list-group-item list-group-item-action' onclick='selectTheater(this, "+"\""+thinfo.thcode+"\""+" )'><span class='listTitle font-bold' title='"+thinfo.thname+"'>"+thinfo.thname+"</span></button>";
					}
				}
				$("#thListArea").html(output);
				if(selectCheck){
		...
		$.ajax({
			...
			success:function(dateList){
				...
				for(var scinfo of dateList){
					...
					sc_year = nowYear;
					sc_month = nowMonth;
					output += "<button type='button' id='"+scinfo.scdate+"' class='list-group-item' onclick='selectDate(this,"+"\""+scinfo.scdate+"\""+")'>";
		...
		$.ajax({
			...
			success:function(roomTimeList){
				...
				for(var scinfo of roomTimeList){
					...
					output += "<button type='button' id='"+scinfo.scroom+scinfo.scdate.replace(':','')+"' class='btn btn-light mb-2' onclick='selectRoomTime(this, "+"\""+scinfo.scroom+"\","+"\""+scinfo.scdate+"\")'>";
